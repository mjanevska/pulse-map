	...
	for(var i=0; i < ven.length; i++){
	  ...
	  // if(inarche === -1){
		  var postParams1 = {
			  ...
			  $.post("services/api/traveltransportcheckin/create", postParams1, function(data, status) {
	    			if (data) {
...
function executeQuery(dbvenues,latlng,rad){
	...
	var queryurl = "https://api.foursquare.com/v2/venues/search?ll="+latlng+"&radius="+rad+"&limit=50&intent=checkin&categoryId=4d4b7105d754a06379d81259&oauth_token=SOJEBSV4JWZZ2E52MUEPOQTL4GMEZSCWTCE5PSRHK5RUUY0N&v=20140609";
	$.ajax({
    	dataType: "json",
    	url: queryurl,
...
//        			console.log(venues[i].hereNow.count);
	        		...
	        		//console.log(venues[i].location.lat.toString());
	        		$.post("services/api/traveltransportvenue/create", postParams, function(data, status) {
	        		    if (data) {
	        		    	//successfull - the venue is returned
	        		    	//_groups[data.id] = data;
    	...
    	// var path = 'services/api/traveltransportcheckin';
   		//  var dbcheckins = [];
...
$(document).ready(function(){
    ...
    $('#btnAddNewGroup').click(function() {
		...
		var path = 'services/api/traveltransportvenue';
		var dbvenues = [];
		var ll = [];
		var rad = [];
    ...
    $('#btnDeleteC').click(function() {
    	...
    	var postParams = {
            ...
            $.post("services/api/traveltransportcheckin/delete", postParams, function(data,status){
...
$('#btnDeleteV').click(function() {
    	...
    	var postParams = {
            ...
            $.post("services/api/traveltransportvenue/delete", postParams, function(data,status){
                //TODO: if successful remove group from the table, else highlight the errors
                    //show returned errors
                    console.log(data);
                    if(data.status === "success"){
